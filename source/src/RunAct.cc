...
RunAct::RunAct(): G4UserRunAction()
{
	...
	// Creating ntuple
	AM -> CreateNtuple("meson_prod", "Meson Production");
	AM -> CreateNtupleIColumn("eventID"); // Column ID = 0
	AM -> CreateNtupleSColumn("creProc"); // Column ID = 1
	AM -> CreateNtupleSColumn("parName"); // Column ID = 2
	AM -> CreateNtupleIColumn("parID"  ); // Column ID = 3
	AM -> CreateNtupleDColumn("k"      ); // Column ID = 4
	AM -> CreateNtupleDColumn("px"     ); // Column ID = 5
	AM -> CreateNtupleDColumn("py"     ); // Column ID = 6
	AM -> CreateNtupleDColumn("pz"     ); // Column ID = 7
	AM -> CreateNtupleDColumn("p"      ); // Column ID = 8
	AM -> CreateNtupleDColumn("cos"    ); // Column ID = 9
	AM -> FinishNtuple();
}
